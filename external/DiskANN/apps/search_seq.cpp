...
std::vector<uint32_t> merge_ids_res(uint32_t seg, std::vector<std::vector<std::vector<uint32_t>>> query_result_ids, 
                                    uint32_t test_id, const uint32_t recall_at, size_t query_num){
    std::vector<uint32_t> ids;
    ids.reserve(query_num * seg * recall_at);
    std::cout << "merging id begin." << std::endl;

    for (size_t i = 0; i < query_num; ++i) {
        for (uint32_t s = 0; s < seg; ++s) {
            const auto& single_query_result = query_result_ids[s][test_id];

            for (uint32_t r = 0; r < recall_at; ++r) {
                uint32_t id = single_query_result[i * recall_at + r] + s * 10000000;
                ids.push_back(id);
            }
        }
    }
    std::cout << "merging id end." << std::endl;
                      ...
                      const std::vector<std::string> &query_filters, const uint32_t segments, const bool use_reorder_data = false)
{
    ...
    std::vector<std::shared_ptr<AlignedFileReader>> readers(segments);
    std::vector<std::unique_ptr<diskann::PQFlashIndex<T, LabelT>>> _pFlashIndices(segments);
    //std::shared_ptr<AlignedFileReader> reader = nullptr;
    
    std::cout << "Segment:" << segments << std::endl;
    ...
    for(int seg = 0; seg < segments; seg++){
        ...
        readers[seg].reset(new WindowsAlignedFileReader());
#else
        readers[seg].reset(new diskann::BingAlignedFileReader());
#endif
#else
        readers[seg].reset(new LinuxAlignedFileReader());
#endif
        _pFlashIndices[seg].reset(new diskann::PQFlashIndex<T, LabelT>(readers[seg], metric));


        // ？？？
        std::string index_path_prefix_i = index_path_prefix + "/graph_" + std::to_string(seg) + "/disk_index_sift10M_R32_L50_A1.2_B0.4";

        int res = _pFlashIndices[seg]->load(num_threads, index_path_prefix_i.c_str());
       ...
       _pFlashIndices[seg]->cache_bfs_levels(num_nodes_to_cache, node_list);

       _pFlashIndices[seg]->load_cache_list(node_list);
        node_list.clear();
    ...
    #pragma omp parallel for schedule(dynamic, 1)
            ...
            {
                _pFlashIndices[seg]->cached_beam_search(warmup + (i * warmup_aligned_dim), 1, warmup_L,
...
//******************************** Search*/
    ...
    std::vector<std::vector<std::vector<uint32_t>>> query_result_ids(segments);
    std::vector<std::vector<std::vector<float>>> query_result_dists(segments);

    for(int i = 0; i < segments; i++){
        query_result_ids[i].resize(Lvec.size());
        query_result_dists[i].resize(Lvec.size());
    }
    ...
    for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++)
    {
        ...
        std::vector<std::vector<uint64_t>> query_result_ids_64(segments);
        //std::vector<std::vector<uint64_t>> query_result_ids_64(recall_at * query_num);


        for(int seg = 0; seg < segments; seg++){

            query_result_ids[seg][test_id].resize(recall_at * query_num);
            query_result_dists[seg][test_id].resize(recall_at * query_num);
            query_result_ids_64[seg].resize(recall_at * query_num);
...
#pragma omp parallel for schedule(dynamic, 1)
            for (int64_t i = 0; i < (int64_t)query_num; i++)
            {
                if (!filtered_search)
                {
                    _pFlashIndices[seg]->cached_beam_search(query + (i * query_aligned_dim), recall_at, L,
                                                    query_result_ids_64[seg].data() + (i * recall_at),
                                                    query_result_dists[seg][test_id].data() + (i * recall_at),
        ...
        for(int seg = 0; seg < segments; seg++){
             diskann::convert_types<uint64_t, uint32_t>(query_result_ids_64[seg].data(), query_result_ids[seg][test_id].data(),
                                                query_num, recall_at);
        }

       
        std::vector<uint32_t> ids_merge = merge_ids_res(segments, query_result_ids, test_id, recall_at, query_num);
        ...
        if (calc_recall_flag)
        {
            recall = diskann::calculate_recall((uint32_t)query_num, gt_ids, gt_dists, (uint32_t)gt_dim,
                                                ids_merge.data(), recall_at*segments, recall_at*segments);
            best_recall = std::max(recall, best_recall);
        }

        std::cout << "Recall:" << best_recall << "  QPS:" << qps << std::endl;
...
//         if (beamwidth <= 0)
//         {
//             std::cout <<"beamwidth <= 0, because beamwidth = " << beamwidth << std::endl;
//            return -1;
//         }
//         else
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        /******** */
        required_configs.add_options()("segment,S",
                                        po::value<uint32_t>(&segments)->required(),
                                        program_options_utils::SEARCH_LIST_DESCRIPTION);
        /******** */
    ...
    if(segments <= 0){
        std::cout << "Segment must be at least 1"
                  << std::endl;
        return -1;
    }
        ...
        if (!query_filters.empty() && label_type == "ushort")
        {
            if (data_type == std::string("float"))
                return search_disk_index<float, uint16_t>(
                    ...
                    num_nodes_to_cache, search_io_limit, Lvec, fail_if_recall_below, query_filters, segments, use_reorder_data);
        ...
        else
        {
            if (data_type == std::string("float"))
                return search_disk_index<float>(metric, index_path_prefix, result_path_prefix, query_file, gt_file,
                                                ...
                                                fail_if_recall_below, query_filters, segments, use_reorder_data);
            else if (data_type == std::string("int8"))
